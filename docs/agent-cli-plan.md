...
| **P1-4** | 工作台应收/应付 与 应收应付页**口径不等** | `dashboard/page.tsx:41-52` 缺 `GREATEST(...,0)` 且无时间范围；`receivables-payables/page.tsx:90-120` 有 | 有超收/超退时两数必然不等 | **已在 dev 数据复现**（§3.6），待按 §13.8 裁决口径 |
...
| **P1-6** | 作废售卖单**不校验已确认退货** | `sale-orders/actions.ts:536-559` | 已部分退货的单再作废 → 按整单回补库存，**多补** | 待修（口径见 §13.9 B4） |
...
- **报表完全不排除估价行**：`estimated` 行 `costAmount = 0`，会让毛利率虚高；`src/lib` 里对 `estimated` 零处引用（报告 05）。⚠️ P0-1 修复后**估价行开始真的产生**，这条从"隐患"升级为"会发生"，需尽快定口径（§13.8）。
- **销售额/毛利口径不冲减退货**：`reports.ts:93-106` 根本不读退货表 → 与"应收应付"口径分叉（报告 03、05）。
...
- **工作台 vs 应收应付页：不追求两数相等，而是各自定义清楚**。工作台的应收/应付是"当前未结清存量"
  （全时间是对的），页面是"区间未结清"。做法：给工作台补 `GREATEST(...,0)`（现在是它错），
  两边都写清语义，CLI 用**不同字段名**（`outstandingTotal` / `outstandingInRange`）而不是同名两个数。
- **厂家对账**：明细行继续读 `payments`（对账单的语义本来就是"收款流水"），**余额汇总改用冗余列**
  （与其它 3 处统一），并加一条"两套不等时告警"——报告 03 说现在两套一致，但这种一致没有守卫，会静默漂移。
- **退货是否冲减销售额/毛利**：CLI **默认给净额**（毛利不冲退货会虚高，这是财务正确性问题），
  同时保留毛额字段；**页面/报表是否改成净额是独立决策**（会切断历史对比），建议另立需求。
- **估价行**：销售额**包含**它（有真实售价），但**成本/毛利必须标注"成本不完整"**，
  并给 `--exclude-estimated` 开关。不要静默扣减——静默扣减会让"报表销售额"和"单据总额"对不上，人会更困惑。
