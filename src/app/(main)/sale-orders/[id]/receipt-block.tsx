    ...
    <div className="rounded-xl border border-gray-200 bg-white p-5">
      ...
      <div className="flex flex-wrap items-center gap-4">
        <div className="text-sm">
          <span className="text-gray-500">已收：</span>
          <span className="font-medium text-gray-900">¥{paidTotal.toFixed(2)}</span>
        </div>
        <div className="text-sm">
          <span className="text-gray-500">未收：</span>
        ...
        {!canPay && <span className="text-xs text-gray-400">收款登记权限：管理员/老板</span>}
      </div>

      {showPayForm && (
        <form action={payAction} className="mt-3 flex flex-wrap items-end gap-2 rounded-lg bg-gray-50 p-3">
          <input type="hidden" name="direction" value="receipt" />
      ...
      <div className="mt-4 space-y-2">
        {payments.length === 0 && <p className="text-sm text-gray-400">暂无收款记录</p>}
        {payments.map((p) => (
